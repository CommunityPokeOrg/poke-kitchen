...
- **Chef names** — the Order Terminal asks "Ordered by" (defaults to a generated name like *Chef Palm 42*, persisted in `localStorage`). Names ride every order payload (`by`), appear on ticket cards and serve logs, and show in the presence tooltip — all HTML-escaped.
